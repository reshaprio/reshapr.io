...
- Prometheus Operator CRDs when enabling the ServiceMonitor; verify them with `kubectl get crd servicemonitors.monitoring.coreos.com`.
...
Confirm that the chart created a ServiceMonitor selecting the proxy service. When the Prometheus Operator CRDs are not installed, keep `serviceMonitor.enabled=false`; the Helm release cannot create that resource, but `/q/metrics` remains available for direct verification.
