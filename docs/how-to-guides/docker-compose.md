...
The CLI downloads the release-owned [`docker-compose-all-in-one.yml`](https://github.com/reshaprio/reshapr/blob/1.0.0/install/docker-compose-all-in-one.yml), updates its reShapr image tags to `1.0.0`, caches it under `~/.reshapr/`, and starts the stack in the background.
