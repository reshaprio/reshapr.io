...
- Node.js 20 or later and reShapr CLI `1.0.0` installed in the job image;
- a reShapr `1.0.0` control plane and a running proxy registered as a Gateway in the default Gateway Group;
...
For the same Service name and version, release `1.0.0` updates the imported Service instead of creating a duplicate. With `--backendEndpoint`, it creates the `default` Configuration Plan and Exposition when absent, then reuses them on later imports.
...
- `reshapr config update` is interactive in `1.0.0` and is unsuitable for an unattended job.
...
The release-tagged [CLI implementation](https://github.com/reshaprio/reshapr/tree/1.0.0/cli) and [public API contract](https://github.com/reshaprio/reshapr/blob/1.0.0/reshapr-public-openapi-v0.1.yaml) remain the canonical interface references.